...
def extract_terms_from_source(source_lines):
    """
    Extract term names from JSON source lines before callout markers.
    
    JSON-specific handling:
    - Key-value pairs: "key": value <1>
    - String values: "value" <1>
    - Array/object markers
    - Preserves full paths like "spec.template.spec"
    ...
    for line_num, line in enumerate(source_lines, start=1):
        ...
        # Pattern 3: Unquoted key (less common but valid in some contexts)
        unquoted_match = re.search(r'([a-zA-Z0-9_\.\-]+)\s*:', pre_marker)
        if unquoted_match:
            term = unquoted_match.group(1)
            terms[marker_num] = term
            continue
        
        # Fallback: extract anything that looks like an identifier
        fallback_match = re.search(r'([a-zA-Z0-9_\.\-]+)', pre_marker)
        if fallback_match:
...
def convert_json_block(full_match, terms, cleaned_source, debug=False):
    """Convert a JSON code block with callouts to definition list format"""
...
def process_file(file_path, debug=False):
    """Process a single AsciiDoc file for JSON callouts"""
    try:
...
def main(list_file_path, debug=False):
    """Main entry point for JSON conversion"""
    ...
    print(f"Processing {len(file_list)} files for JSON callout conversion...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert AsciiDoc JSON callouts to definition lists.")
