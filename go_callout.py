...
CONVERTIBLE_LANGUAGES = ['go', 'golang']

def get_go_block_pattern():
    return re.compile(
        r'(\[source,([a-zA-Z0-9_-]+).*?\n)'
        r'(\s*-{4,}\s*\n)'
        r'(.*?)\s*-{4,}\s*\n'
...
def extract_terms_from_source(source_lines):
    """
    Extract term names from Go source lines before callout markers.
    
    Go-specific handling:
    - Variable declarations: var name type <1>
    - Function definitions: func FunctionName() <1>
    - Type definitions: type MyType struct <1>
    - Package imports: import "package" <1>
    - Struct fields: FieldName string <1>
    ...
    for line_num, line in enumerate(source_lines, start=1):
        ...
        if func_match:
            term = func_match.group(1)
            terms[marker_num] = term
            continue
        
        # Pattern 2: Type definition
        type_match = re.search(r'type\s+([A-Za-z_][A-Za-z0-9_]*)', pre_marker)
        if type_match:
            term = type_match.group(1)
            terms[marker_num] = term
            continue
        
        # Pattern 3: Variable declaration (var or :=)
        var_match = re.search(r'(?:var\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*(?::=|=|[A-Za-z])', pre_marker)
        if var_match:
        ...
        import_match = re.search(r'import\s+(?:"([^"]+)"|([A-Za-z_][A-Za-z0-9_]*))', pre_marker)
        if import_match:
            term = import_match.group(1) or import_match.group(2)
            terms[marker_num] = term
            continue
        
        # Pattern 5: Struct field or method
        field_match = re.search(r'([A-Za-z_][A-Za-z0-9_]*)\s+[A-Za-z_*\[\]]', pre_marker)
        if field_match:
            term = field_match.group(1)
            terms[marker_num] = term
            continue
        
        # Pattern 6: Function/method call
...
def convert_go_block(full_match, terms, cleaned_source, debug=False):
    """Convert a Go code block with callouts to definition list format"""
...
def process_file(file_path, debug=False):
    """Process a single AsciiDoc file for Go callouts"""
    try:
...
def main(list_file_path, debug=False):
    """Main entry point for Go conversion"""
    ...
    print(f"Processing {len(file_list)} files for Go callout conversion...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert AsciiDoc Go callouts to definition lists.")
